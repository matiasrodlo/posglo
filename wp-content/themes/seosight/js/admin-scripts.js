...
jQuery(document).ready(function ($) {
    ...
    if (jQuery('body').hasClass('post-type-post')) {
        ...
        var seosight_pf_selected = $selector_panel.find('input:radio[name=post_format]:checked').val();
        $post_format_metaboxes.hide(); // Default Hide
        jQuery('#fw-options-box-post-' + seosight_pf_selected).show();
        $selector_panel.find('input:radio[name=post_format]').change(function () {
            $post_format_metaboxes.hide(); // Hide during changing
            seosight_pf_selected = $selector_panel.find('input:radio[name=post_format]:checked').val();
            if (this.value == seosight_pf_selected) {
                jQuery('#fw-options-box-post-' + seosight_pf_selected).show();
            }
        });
    }
