...
el_classess = ['kc_image_gallery',
	...
	if (click_action === 'lightbox'){
		el_classess.push('js-zoom-gallery');
	}
...
if( attachment_data[0] === undefined || attachment_data[0] ==='' ){
	...
	for( var i=0; i< attachment_data.length; i++){
		...
		switch( click_action ){
			...
			case 'lightbox':
				output += '<div class="item-grid grid-' + columns + '"><a class="kc-image-link" data-lightbox="gallery-item" href="' + attachment_data_full[i] + '">';
				output += '<img src="' + image + '" /></a></div>';
				break;
...
<div class="kc-image-gallery {{{wrp_class}}}">
	<# if( title !== '' ){#>
	<div class="crumina-heading heading-title">
